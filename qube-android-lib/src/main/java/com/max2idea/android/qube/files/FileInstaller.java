...
public class FileInstaller {
    ...
    public static void installFiles(Activity activity, boolean force) {
        ...
        File tmpDir = new File(QubeApplication.getBasefileDir());
        if (!tmpDir.exists()) {
            if(tmpDir.mkdirs()) {
        ...
        File tmpDir1 = new File(QubeApplication.getMachineDir());
        if (!tmpDir1.exists()) {
            if(tmpDir.mkdirs()) {
        ...
        //Install base dir
        File dir = new File(QubeApplication.getBasefileDir());
        if (dir.exists() && dir.isDirectory()) {
            //don't create again
        } else if (dir.exists() && !dir.isDirectory()) {
            Log.e(TAG, "Could not create Dir, file found: " + QubeApplication.getBasefileDir());
            return;
        } else if (!dir.exists()) {
            dir.mkdir();
        }

        String destDir = QubeApplication.getBasefileDir();
        ...
        for (int i = 0; i < files.length; i++) {
            ...
            if (subfiles != null && subfiles.length > 0) {
                //Install base dir
                File dir1 = new File(QubeApplication.getBasefileDir() + files[i]);
                if (dir1.exists() && dir1.isDirectory()) {
                    //don't create again
                } else if (dir1.exists() && !dir1.isDirectory()) {
                    Log.e(TAG, "Could not create Dir, file found: " + QubeApplication.getBasefileDir() + files[i]);
                    return;
                } else if (!dir1.exists()) {
                    dir1.mkdir();
                }
                ...
                if(!file.exists() || force) {
                    Log.d(TAG, "Installing file: " + file.getPath());
                    installAssetFile(activity, files[i], QubeApplication.getBasefileDir(), "roms", null);
                }
            }
        }
    }
    ...
    public static boolean installAssetFile(Context context, String srcFile,
        ...
        try {
            ...
            if (!destDirF.exists() && !destDirF.mkdirs()) {
                ToastUtils.toastShort(context, context.getString(R.string.CouldNotCreateDirForImage));
                is.close();
                return false;
            }
            if (destFile == null) {
            ...
            OutputStream os = new FileOutputStream(new File(destDirF, destFile));
            byte[] buf = new byte[8092];
            int n;
            while ((n = is.read(buf)) > 0) {
                os.write(buf, 0, n);
