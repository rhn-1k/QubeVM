...
public class FavOpenHelper extends SQLiteOpenHelper {
    ...
    synchronized boolean deleteAllFavs() {
        try {
            db.delete(TABLE_NAME_FAV_FILES, null, null);
            return true;
        } catch (Exception e) {
            Log.w(TAG, "Error while deleting all favs: " + e.getMessage());
            return false;
        }
    }

    synchronized boolean deleteFavsByType(String favType) {
        try {
            db.delete(TABLE_NAME_FAV_FILES, FAVFILETYPE + " = ?", new String[]{favType});
            return true;
        } catch (Exception e) {
            Log.w(TAG, "Error while deleting favs by type: " + e.getMessage());
            return false;
        }
    }
}
