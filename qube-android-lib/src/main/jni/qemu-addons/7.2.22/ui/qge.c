    ...
    uint32_t h)
{
    ...
    if (!qube_gfx_surface || surface_width(qube_gfx_surface) != (int)backing_width ||
        ...
        dpy_gfx_replace_surface(dcl->con,
...
static void qube_gfx_cursor_define(DisplayChangeListener *dcl, QEMUCursor *cursor)
{
    qemu_mutex_lock(&qube_gfx_lock);
    if (qube_gfx_cursor) {
        cursor_put(qube_gfx_cursor);
    }
    if (cursor) {
        cursor_get(cursor);
    }
    qube_gfx_cursor = cursor;
    ...
    uint32_t h)
{
    ...
    qatomic_inc(&qube_gfx_generation);
    qemu_mutex_unlock(&qube_gfx_lock);
    dpy_gfx_update(dcl->con, x, y, w, h);
}
...
static bool qube_gfx_egl_init(void)
{
    ...
    qemu_egl_mode = DISPLAYGL_MODE_ES;

    // Created once here, shared by every context virgl creates below
    {
        static const EGLint root_ctx_att[] = {
            EGL_CONTEXT_CLIENT_VERSION, 3,
            EGL_NONE,
        };
        qube_gfx_root_ctx = eglCreateContext(qemu_egl_display,
            qemu_egl_config, EGL_NO_CONTEXT, root_ctx_att);
...
{
    EGLint ctx_att[] = {
        EGL_CONTEXT_CLIENT_VERSION, params ? params->major_ver : 2,
        EGL_NONE,
    };
    return (QEMUGLContext)eglCreateContext(qemu_egl_display,
        qemu_egl_config, qube_gfx_root_ctx, ctx_att);
}
...
{
    graphic_hw_update(qube_gfx_con);
...
void qube_gfx_init(QemuConsole *con)
{
    ...
    register_displaychangelistener(&qube_gfx_dcl);
